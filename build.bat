...
REM
REM 产物：
REM   dist/XHGan/XHGan.exe              （绿色版文件夹）
REM   dist/XHGan_Setup_x64.exe          （Inno Setup 安装向导，若已装 Inno Setup）
...
if errorlevel 1 (
    ...
    echo        绿色版位置: dist\XHGan\XHGan.exe
) else (
    iscc installer.iss
    if errorlevel 1 (
    ...
    echo 安装向导生成完毕: dist\XHGan_Setup_x64.exe
)
